...
var spine,stgbutton,stgbutton1,
  spineGroup,
  spine1img,
  spine2img,
  spineimg,
...
function setup() {
  ...
  kpf.position(width / 2 + 300, height/2);
  kpf.size(225, 225);
  kpf.class("plimg1");
  kpf.hide();
  ...
  portal.position( width / 2 - -260,height/2 - -120);
  portal.size(200, 200);
  portal.class("plimg1");
  portal.hide();
  ...
  stgbutton= createImg("start the game.png");
  stgbutton.position(width / 2 - 150 , height /2);
  stgbutton.size(300, 150);
  stgbutton.hide();

    stgbutton1= createImg("start the game.png");
  stgbutton1.position(width / 2 - 150 , height /2);
  stgbutton1.size(300, 150);
  stgbutton1.hide();
  stgbutton.class("plimg1")
...
csb.visible = true;
    stgbutton1.show();
      ...
      icon: "info",
      title: "How to play this game ?",
      text: "You need to take the hint paper without touching the obstacles(spine and caterpillar) . Click the space bar for jumping and left arrow to move left and right arrow for moving right",
    }).then((result) => {
      if (result.isConfirmed) {
        climber.play(); 
        gameState = "lvl2wait"
...
csb.visible = true;
  ...
  solvebt.mousePressed(() => {
        Swal.fire({
      icon: "info",
      title: "Instructions for solving puzzle",
      text: "There will be a puzzle to solve and there will be no time limit . If you click on the wrong answer the life will be decreased by 1 . Click on the correct answer to know the second clue .",
    }).then((result) => {
      if (result.isConfirmed) {
